...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, mask=None, freq=None):
        return_ids = []
        return_feats = []
        ...
        for feat_id, feat in enumerate(feats):
            ...
            assert H in [518, 512, 256, 128]
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)   # 1, 268324, 3

            if num_patches > 0:
                if patch_ids is not None:
                    patch_id = patch_ids[feat_id]
                else:
                    if mask is None:    # 随机采样
                        # torch.randperm produces cudaErrorIllegalAddress for newer versions of PyTorch. https://github.com/taesungp/contrastive-unpaired-translation/issues/83
                        #patch_id = torch.randperm(feat_reshape.shape[1], device=feats[0].device)
                        patch_id = np.random.permutation(feat_reshape.shape[1])
                        patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
                    else:    
                        reshaped_mask, reshaped_freq = self.resize_masks(H, W, mask, freq)          

                        mask_reshape = reshaped_mask.flatten(1, 2)     # 1, 268324
                        assert mask_reshape.shape[1] == feat_reshape.shape[1]
                        if freq is not None:
                            freq_reshape = reshaped_freq.flatten(1, 2)

                        cancer_indices = torch.where(mask_reshape[0] == 1)[0].cpu()           # BIO获得的癌症区域

                        if freq is None:    
                            # 如果不采用DINO，直接以预先设定的比例分别对癌症、非癌症区域进行采样
                            p = torch.full((feat_reshape.shape[1],), self.opt.non_cancer_prob) # 首先默认全部位置的采样频率都是non_cancer_prob
                            p[cancer_indices] = self.opt.cancer_prob                    # 将癌症区域的采样频率改为cancer_prob
                            weighted_p = p / torch.sum(p) # 归一化所有位置的权重
                            patch_id = np.random.choice(feat_reshape.shape[1], size=num_patches, replace=False, p=weighted_p.numpy())    # 按照概率采样得到的索引（从全序列采样）
                        else:
                            if len(cancer_indices) == 0:    # 没有癌症区域，纯随机取
                                patch_id = np.random.permutation(feat_reshape.shape[1])
                                patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]
                            else:
                                bio_dino_max = torch.max(freq_reshape[:, cancer_indices])   # 癌症区域最大的DINO的取值
                                # 随机从bio_dino_max_indices里采样dino_sample个点
                                bio_dino_max_indices = torch.where((mask_reshape[0] == 1) & (freq_reshape[0] == bio_dino_max))[0].cpu().tolist()
                                # print(len(bio_dino_max_indices), torch.sum(freq_reshape[0] == bio_dino_max))
                                patch_id1 = np.random.choice(bio_dino_max_indices, size=min(self.opt.dino_sample, len(bio_dino_max_indices)), replace=False).tolist()
                                # 从其他区域采样剩下的点
                                remian_indices = list(set(range(feat_reshape.shape[1])).difference(set(bio_dino_max_indices)))
                                patch_id2 = np.random.choice(remian_indices, size=num_patches - len(patch_id1), replace=False).tolist()
                                # 合并
    ...
    def resize_masks(self, H, W, mask, freq):
        assert H == W

        def resize(input, H):
            if H == 518:
                reshaped_input = F.pad(input, (3, 3, 3, 3), 'constant', 0)
            elif H == 512:
                reshaped_input = input[:,::,::]
            elif H == 256:
                reshaped_input = input[:, ::2, ::2]
            elif H == 128:
                reshaped_input = input[:, ::4, ::4]
            return reshaped_input

        reshaped_mask = resize(mask, H)
        assert reshaped_mask.shape[1] == H  and reshaped_mask.shape[2] == W

        if freq is not None:
            reshaped_freq = resize(freq, H)
            assert reshaped_mask.shape == reshaped_freq.shape
        else:
            reshaped_freq = None

        return reshaped_mask, reshaped_freq
...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input, labels=None, return_feat=False):
        """Standard forward."""
